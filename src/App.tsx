  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col h-full mt-2">
        ...
        <div className="flex gap-2 mt-auto" onPointerDown={e => e.stopPropagation()}>
          <button
            onClick={() => onSelect(f, 'timetable')}
            className="flex-[2] bg-blue-600 hover:bg-blue-700 text-white py-3 px-4 rounded-2xl font-bold shadow-lg shadow-blue-100 transition-all active:scale-95 flex items-center justify-center gap-2 text-sm"
          >
          ...
          <button
            ...
            className="flex-1 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-900 p-3 rounded-2xl transition-all active:scale-95 flex items-center justify-center"
            title="월간 달력"
          >
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className="p-4 max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6 bg-white p-4 rounded-2xl border border-slate-200">
          <div className="flex items-center gap-4">
            <button onClick={() => setCurrentView('list')} className="p-2 hover:bg-slate-100 rounded-full"><ChevronLeft size={20} /></button>
            <div><h2 className="text-xl font-bold text-slate-800">{selectedFacility.name}</h2><p className="text-slate-400 text-[10px] font-medium leading-none">월간 현황</p></div>
          </div>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4">
              <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))} className="p-2 hover:bg-slate-100 rounded-xl"><ChevronLeft size={20} /></button>
              <span className="font-black text-xl text-slate-800 w-32 text-center">{year}년 {month + 1}월</span>
              <button onClick={() => setCurrentDate(new Date(year, month + 1, 1))} className="p-2 hover:bg-slate-100 rounded-xl"><ChevronRight size={20} /></button>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setCurrentView('timetable')} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-bold">주간 보기</button>
              <button onClick={() => setCurrentView('list')} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-bold">홈으로</button>
            </div>
          </div>
        </div>
              ...
              return (
                <div key={i} className="border-r border-b border-slate-50 p-2 hover:bg-slate-50 transition-all cursor-pointer" onClick={() => { setCurrentDate(date); setCurrentView('timetable'); }}>
                  ...
                  <div className="space-y-1">
                    {dayRes.map((res, idx) => {
                      const color = getColorClass(res.teacherName);
                      return <div key={idx} className={`text-[10px] font-black ${color.bg} ${color.text} px-1.5 py-0.5 rounded-lg truncate`}>{res.reason}</div>
                    })}
                  </div>
                </div>
              );
  ...
  const renderGlobalCalendar = () => {
    ...
    return (
      <div className="p-4 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6 bg-white p-4 rounded-2xl border border-slate-200 text-center">
          <div className="flex items-center gap-4">
            <button onClick={() => setCurrentView('list')} className="p-2 hover:bg-slate-100 rounded-full"><ChevronLeft size={20} /></button>
            <h2 className="text-xl font-black text-slate-800">전체 예약 현황</h2>
          </div>
          <div className="flex items-center gap-6">
            <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))}><ChevronLeft /></button>
            <span className="font-black text-xl">{year}년 {month + 1}월</span>
            <button onClick={() => setCurrentDate(new Date(year, month + 1, 1))}><ChevronRight /></button>
            <button onClick={() => setCurrentView('list')} className="px-6 py-2 bg-slate-900 text-white rounded-xl text-sm font-bold">목록으로</button>
          </div>
        </div>
